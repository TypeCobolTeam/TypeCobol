...
namespace TypeCobol.Analysis.Cfg
{
    ...
    public class ControlFlowGraphBuilder<D> : ProgramClassBuilderNodeListener
    {
        ...
        /// </summary>
        /// <param name="p">The Perfomr Procedure node source of the call</param>
        /// <param name="group">The Group to be relocate</param>
        /// <param name="clonedBlockIndexMap">The map between the old block indices to their new block indices.</param>
        /// <returns>true if the relocation is successfull, fals eif the relocation goes beyond the group limit.</returns>
            ...
            foreach (var b in group.Group)
            {
                ...
                foreach (var edge in successors)
                {
                    ...
                    if (!clonedEdgeIndexMap.TryGetValue(edge, out newEdge))
                    {
                        ...
                        {   //Hum this Group is not the last group and it goes beyond the group limit ==> we don'it support that.
                            if (b != group.Group.Last.Value)
                            {
                                Diagnostic d = new Diagnostic(MessageCode.SemanticTCErrorInParser,
                                    ...
                                    p.CodeElement.Column,
                                    p.CodeElement.Line,
                                    string.Format(Resource.BasicBlockGroupGoesBeyondTheLimit, group.Tag.ToString(), block.Index));
                                Diagnostics.Add(d);
