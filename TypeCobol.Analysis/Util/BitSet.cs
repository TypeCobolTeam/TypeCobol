...
//--------------------------------------------------------------------------------------------------------------
//MAYANJE:
//This code is derivated from ANTLR source code: at
//https://github.com/antlr/antlr4/blob/master/runtime/CSharp/runtime/CSharp/Antlr4.Runtime/Sharpen/BitSet.cs
//See file antrl4_license.txt in the same directory for the BSD 3-clause license
//--------------------------------------------------------------------------------------------------------------
// Added: Not, Xor, Union, Intersection, Difference operations.
//--------------------------------------------------------------------------------------------------------------
namespace TypeCobol.Analysis.Util
{
    public class BitSet
    {
        ...
        /// </summary>
        /// <param name="set">The set to be unioned with this.</param>
        ...
        public BitSet Union(BitSet set)
        {
            ...
            if (_data.Length < set._data.Length)
                return set.Union(this);

            BitSet result = new BitSet(new ulong[_data.Length]);

            for (int i = 0; i < set._data.Length; i++)
                result._data[i] = _data[i] | set._data[i];
        ...
        /// </summary>
        /// <param name="set">The other set to be intersected with this.</param>
        /// <returns>The intersection set</returns>
        public BitSet Intersection(BitSet set)
        {
            if (set == null)
                throw new ArgumentNullException("set");
            ...
            BitSet result = new BitSet(new ulong[set._data.Length]);
        ...
        /// An element is in the difference if it is in this, but not in the other set
        /// </summary>
        ...
        public BitSet Difference(BitSet set)
        {
            ...
            else
            {
                ...
                for (int i = set._data.Length; i < _data.Length; i++)
                {
