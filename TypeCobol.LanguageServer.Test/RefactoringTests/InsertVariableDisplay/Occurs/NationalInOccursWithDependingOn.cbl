...
refactoring.source=
      ...
      *<DBG>InsertVariableDisplay 1959/09/18 11:09 TESTUSER
      D77 Idx-d6b813ab-1 PIC 9(4) COMP.
      D77 Idx-d6b813ab-2 PIC 9(3) COMP.
      D77 Idx-d6b813ab-3 PIC 9(2) COMP.
      *</DBG>

       PROCEDURE DIVISION.
      *<DBG>InsertVariableDisplay 1959/09/18 11:09 TESTUSER
      D    IF root-table-3-count >= 1 AND <= 1000
      D      PERFORM VARYING Idx-d6b813ab-1 FROM 1 BY 1 UNTIL
      D      Idx-d6b813ab-1 > root-table-3-count
      D        IF table-level1-2-count >= 1 AND <= 200
      D          PERFORM VARYING Idx-d6b813ab-2 FROM 1 BY 1 UNTIL
      D          Idx-d6b813ab-2 > table-level1-2-count
      D            IF table-level2-1-count >= 1 AND <= 30
      D              PERFORM VARYING Idx-d6b813ab-3 FROM 1 BY 1 UNTIL
      D              Idx-d6b813ab-3 > table-level2-1-count
      D                DISPLAY '        var-national (' Idx-d6b813ab-1
      D                ' ' Idx-d6b813ab-2 ' ' Idx-d6b813ab-3 ') <'
      D                FUNCTION DISPLAY-OF (var-national (Idx-d6b813ab-1
      D                Idx-d6b813ab-2 Idx-d6b813ab-3)) '>'
      D              END-PERFORM
      D            ELSE
