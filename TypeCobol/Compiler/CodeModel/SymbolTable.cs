...
namespace TypeCobol.Compiler.CodeModel {
    ...
    public class SymbolTable {
        ...
        private List<T> GetFromTableAndEnclosing<T>(string name, Func<SymbolTable, IDictionary<string, List<T>>> getTableFunction) where T : Node
        {
            var table = getTableFunction.Invoke(this);
            var values = GetFromTable(name, table);
            if (EnclosingScope != null)
            {
                values.AddRange(EnclosingScope.GetFromTableAndEnclosing(name, getTableFunction));
            }
            return values;
        }

        private List<T> GetFromTable<T>(string name, IDictionary<string, List<T>> table) where T : Node
        {
            List<T> values;
            table.TryGetValue(name, out values);
        ...
        /// </summary>
        public IDictionary<string,List<DataDefinition>> DataEntries = new Dictionary<string,List<DataDefinition>>(StringComparer.InvariantCultureIgnoreCase);
        ...
        private IList<DataDefinition> GetVariable(string name) { return GetFromTableAndEnclosing(name, GetDataDefinitionTable); }
	    ...
	    private IList<DataDefinition> GetVariablesTyped(QualifiedName typename) {
		    ...
		    foreach(var items in DataEntries.Values) {
			    foreach(var item in items) {
				    if (typename.Head.Equals(item.DataType.Name, System.StringComparison.InvariantCultureIgnoreCase)) variables.Add(item);
			    }
		    }
		    if (EnclosingScope!= null)
	    ...
	    private IDictionary<string,List<Section>> Sections = new Dictionary<string,List<Section>>(StringComparer.InvariantCultureIgnoreCase);

	    internal void AddSection(Section section) { Add(Sections, section); }

        public IList<Section> GetSection(string name) { return GetFromTableAndEnclosing(name, GetSectionTable); }
        ...
        private IDictionary<string,List<Paragraph>> Paragraphs = new Dictionary<string,List<Paragraph>>(StringComparer.InvariantCultureIgnoreCase);

	    internal void AddParagraph(Paragraph paragraph) { Add(Paragraphs, paragraph); }

	    public IList<Paragraph> GetParagraph(string name) { return GetFromTableAndEnclosing(name, GetParagraphTable); }
        ...
        public IDictionary<string,List<TypeDefinition>> Types = new Dictionary<string,List<TypeDefinition>>(StringComparer.InvariantCultureIgnoreCase);
	    ...
	    private List<TypeDefinition> GetType(string name) { return GetFromTableAndEnclosing(name, GetTypeTable); }
        ...
        public IDictionary<string,List<FunctionDeclaration>> Functions = new Dictionary<string,List<FunctionDeclaration>>(StringComparer.InvariantCultureIgnoreCase);
    ...
    //		if (p1.ReturningParameter != p2.ReturningParameter) return false;
            ...
            [NotNull]
	    private List<FunctionDeclaration> GetFunction(string name) { return GetFromTableAndEnclosing(name, GetFunctionTable); }
