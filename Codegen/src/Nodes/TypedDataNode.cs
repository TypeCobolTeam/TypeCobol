...
namespace TypeCobol.Codegen.Nodes
{
    ...
    internal class TypedDataNode : DataDescription, Generated
    {
        ...
        public static List<ITextLine> InsertChildren(ColumnsLayout? layout, List<string> rootProcedures, List< Tuple<string,string> > rootVariableName, DataDefinition ownerDefinition, DataDefinition type, int level, int indent)
        {
            ...
            // List of all the CopyDirectives that has been added to the lines
            ...
            foreach (var child in type.Children)
            {
                ...
                if (child.IsFlagSet(Flag.InsideTypedefFromCopy))
                {
                    if (child.IsFlagSet(Flag.IsTypedefCopyNode))
                    {
                        ...
                        {
                            if (i == 0 && child.Lines.ElementAt(i).Text.Count(c => c == '.') > 1)
                            {
                                var spacing = child.Lines.ElementAt(i).Text.TakeWhile(char.IsWhiteSpace).Count();
                                lines.Add(new CobolTextLine(new TextLineSnapshot(child.Lines.ElementAt(i).LineIndex, new string(' ', spacing) + child.CodeElement.Text, null), ColumnsLayout.CobolReferenceFormat));
                            }
                            else
                            {
                                lines.Add(child.Lines.ElementAt(i));
                            }
                        }
                        continue;
                    }
                ...
                if (bIsInsideCopy && child.CodeElement != null)
                {
                    ...
                    if (type.Root != null && !type.Root.Programs.Any(p => rootProcedures.Contains(p.Name)))
                    {
                        ...
                        if (usedCopies.Contains(copy) == false)
                        {
                            ...
                            copyInstruction.Append(' ', Math.Max(GetLastLine(child).Text.TakeWhile(char.IsWhiteSpace).Count() - 7, copy.COPYToken.TokensLine.SourceText.TakeWhile(char.IsWhiteSpace).Count()));
        ...
        private static ITextLine GetLastLine(Node parent)
        {
